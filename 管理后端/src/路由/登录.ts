...
export function 创建登录路由(写限流: RequestHandler): Router {
  ...
  路由.post('/deng-lu', 写限流, async (请求: Request, 响应: Response): Promise<void> => {
    ...
    const 手机号 = 校验手机号('手机号', 取可选字符串(正文['shou_ji_hao']));
    const 密码 = 取必填字符串(取可选字符串(正文['mi_ma']), '密码', 200);
    ...
    const 行 = await 池.query(
      'SELECT "ID", "手机号", "用户名", "密码哈希", "管理员" FROM "用户" WHERE "手机号" = $1 LIMIT 1',
      [手机号],
    );
    if (行.rows.length === 0) {
    ...
    // YH-031 签发算法白名单HS256显式声明
    const 令牌 = jwt.sign(
      { yongHuId: 用户编号, guanLi: true, qianFaHaoMiao: Date.now() },
      当前配置().令牌密钥,
  ...
  路由.post('/shua-xin', 写限流, async (请求: Request, 响应: Response): Promise<void> => {
    ...
    if (目标编号 === null) {
      失败响应(响应, 401, 取文案('登录', '刷新令牌无效'), 'LING_PAI_WU_XIAO');
      return;
    }
    try {
    ...
    const 查 = await 池.query('SELECT "管理员" FROM "用户" WHERE "ID" = $1 LIMIT 1', [目标编号]);
    if (查.rows.length === 0 || 查.rows[0]['管理员'] !== true) {
      失败响应(响应, 401, 取文案('登录', '刷新令牌无效'), 'LING_PAI_WU_XIAO');
      return;
    }
    const 令牌有效期 = 当前配置().令牌有效期;
    const 新令牌 = jwt.sign(
      { yongHuId: 目标编号, guanLi: true, qianFaHaoMiao: Date.now() },
      当前配置().令牌密钥,
    ...
    成功响应(响应, { yong_hu_id: 目标编号, guan_li: true });
  });
