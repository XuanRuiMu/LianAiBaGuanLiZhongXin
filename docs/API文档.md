...
路径与查询/包体键一律拼音（协议不动）；下表「说明」里出现的动作名与区块名一律取 `docs\术语表.md` 的唯一显示名，与界面一致。
...
| GET | /api/guan-li/duo-she-ri-zhi | 接管记录分页 |
...
| POST | /api/guan-li/feng-jin | 提交封禁（事务审计） |
| GET | /api/guan-li/zhang-hao-feng-jin | 账号封禁分页总数（替代200截断） |
| POST | /api/guan-li/zhang-hao-feng-jin/jie-feng | 解除封禁（影响行断言） |
...
| POST | /api/guan-li/ju-bao-yi-shen | 举报初审 |
| POST | /api/guan-li/ju-bao-er-shen | 举报复审 |
| POST | /api/guan-li/ju-bao-pi-liang | 举报处理多项（同态原子） |
| GET | /api/guan-li/gong-dan-lie-biao | 工单分页 |
| POST | /api/guan-li/gong-dan-xin-jian | 工单新建（SLA48小时） |
| POST | /api/guan-li/gong-dan-yi-shen | 工单初审 |
| POST | /api/guan-li/gong-dan-er-shen | 工单复审 |
| POST | /api/guan-li/gong-dan-pi-liang | 工单处理多项 |
| GET | /api/guan-li/gong-gao-lie-biao | 公告分页 |
| POST | /api/guan-li/gong-gao-xin-jian | 公告新建（SLA24小时） |
| POST | /api/guan-li/gong-gao-yi-shen | 公告初审 |
| POST | /api/guan-li/gong-gao-er-shen | 公告复审 |
| POST | /api/guan-li/gong-gao-pi-liang | 公告处理多项 |
...
| POST | /api/guan-li/huo-dong-yi-shen | 活动初审 |
| POST | /api/guan-li/huo-dong-er-shen | 活动复审 |
| POST | /api/guan-li/huo-dong-pi-liang | 活动处理多项 |
...
| POST | /api/guan-li/shi-yan-yi-shen | 实验初审 |
| POST | /api/guan-li/shi-yan-er-shen | 实验复审 |
| POST | /api/guan-li/shi-yan-pi-liang | 实验处理多项 |
| POST | /api/guan-li/shi-yan-fa-bu | 实验发布结束 |
| GET | /api/guan-li/liu-hen | 处理记录分页 |
| POST | /api/guan-li/shou-quan | 授予角色（事务二次确认） |
| POST | /api/guan-li/hui-shou | 回收角色（事务二次确认） |
| POST | /api/guan-li/duo-she | 接管角色（事务审计） |
| POST | /api/guan-li/gui-huan | 结束接管（事务审计） |
