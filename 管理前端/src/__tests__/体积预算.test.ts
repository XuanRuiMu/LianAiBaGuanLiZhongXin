...
// FP-17 本轮实测：App.vue 登录态分流类 + 登录页居中几何令首屏 index 分块由 63017 升到 63022（裁定值 64690 未动）
const 首屏实测字节 = 63_022;
// FP-17 本轮实测：色方案双档/滚动条五档/Firefox 分支/autofill 兜底/登录居中令全站 gzip 由 96065 升到 96330，
// 仍锁在裁定值 96460 之内（余 130B）；裁定值 全站预算字节 未动
const 全站实测字节 = 96_330;
...
describe('FP-09C 构建体积预算', () => {
  ...
  it('全站 js+css 分块 gzip 之和不超过裁定值 96.46kB，且锁死在本轮实测值', () => {
