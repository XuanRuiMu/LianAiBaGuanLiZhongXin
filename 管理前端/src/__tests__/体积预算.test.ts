...
const 首屏预算字节 = 64_690;
const 全站预算字节 = 97_200;
const 首屏原始预算字节 = 171_530;
// FP-03 本轮实测：会话状态机（单飞续期 + 跨重开持久标记）与登录三选项令首屏 index 由 63022 升到 63374（+352B），裁定值 64690 未动
const 首屏实测字节 = 63_374;
// FP-03 本轮实测：同一批特性令全站 gzip 由 96330 升到 97030（+700B，其中全站裁定值首次由 96460 抬到 97200，
// 抬幅 = 实测 + 170B 余量，由主代理按 skill『基线常量变更』放行权批准并记入交付报告；裁掉这 700B 需要重构 axios 或
// 把 列定义→枚举映射 聚合改成按视图切分，二者都超出本功能点范围，不做隐性偷裁）
const 全站实测字节 = 97_030;
