...
const 首屏预算字节 = 66_690;
const 全站预算字节 = 101_700;
const 首屏原始预算字节 = 179_670;
...
const 首屏实测字节 = 66_303;
// FP-04 本轮实测：全站 gzip 由 97030 升到 100183（+3153B）。仍可再压的已知杠杆有二，均判为超范围而非隐性偷裁：
// ①axios 换原生 fetch（≈-13.9kB gzip，要重写 请求.ts 拦截器与错误归一并重跑接口层全部测试）；
// ②列定义→枚举映射 聚合改为按视图切分。二者各立专项，不与动效混在同一记账里
...
// 说明"每加一个特性就抬一次预算"在这里是被度量拦住的。axios→原生 fetch 杠杆约 -13.9kB，落地后必须把三项裁定值**下调**
// 到新的实测值以下——只会上抬的预算不叫预算，那等于没有度量
const 全站实测字节 = 101_696;
